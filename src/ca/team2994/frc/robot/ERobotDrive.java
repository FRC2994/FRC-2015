...
public class ERobotDrive implements MotorSafety {
	...
	/*
	 * Driving functions These functions provide an interface to multiple motors
	 * that is used for C programming The Drive(speed, direction) function is
	 * the main part of the set that makes it easy to set speeds and direction
	 ...
	 */
	public ERobotDrive(SpeedController frontLeftMotor,
			SpeedController rearLeftMotor, SpeedController centerLeftMotor,
			SpeedController centerRightMotor, SpeedController frontRightMotor,
	...
	/**
	 * Common function to initialize all the robot drive constructors. Create a
	 * motor safety object (the real reason for the common code) and initialize
	 * all the motor assignments. The default timeout is set for the robot
	 * drive.
	 */
	...
	public void drive(double outputMagnitude, double curve) {
		...
		if (!driveReported) {
			UsageReporting.report(tResourceType.kResourceType_RobotDrive,
					getNumMotors(), tInstances.kRobotDrive_ArcadeStandard);
			driveReported = true;
		}
	 ...
	 *            direction
	 * @param moveAxis
	 *            The axis on the moveStick object to use for fowards/backwards
	 *            (typically Y_AXIS)
	 * @param rotateStick
	...
	public void arcadeDrive(GenericHID moveStick, int moveAxis,
			GenericHID rotateStick, int rotateAxis, boolean squaredInputs) {
	 ...
	 * motors are set to "leftOutput" and "rightOutput" and includes flipping
	 * the direction of one side for opposing motors.
	 * 
	 * @param leftOutput
